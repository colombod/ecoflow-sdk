"""Tests for the MqttTransport implementation."""
...
from ecoflow.transport.mqtt import MessageCallback, MqttCredentials, MqttTransport
...
async def test_subscribe_registers_callback() -> None:
    ...
    assert "SN12345" in client.subscriptions
...
async def test_message_routing_calls_correct_callback() -> None:
    ...
    await client.dispatch_message("/open/user456/SN12345/quota", {"soc": 85})
...
async def test_message_routing_ignores_wrong_sn() -> None:
    ...
    received: list[dict[str, Any]] = []
    client.on_message("SN12345", lambda sn, data: received.append(data))
    await client.dispatch_message("/open/user456/DIFFERENT_SN/quota", {"soc": 50})
    assert received == []
...
async def test_connect_times_out_when_broker_unreachable() -> None:
    ...
    client = MqttTransport(MQTT_CREDS, connect_timeout=0.1)
...
async def test_on_message_default_topic_template_is_open_quota() -> None:
    ...
    _template, _callbacks = client.subscriptions["SN12345"]
...
async def test_subscriptions_store_tuple_with_template_and_callbacks() -> None:
    """subscriptions maps SN → (topic_template, [callbacks])."""
    client = MqttTransport(MQTT_CREDS)
    cb: MessageCallback = lambda sn, data: None  # noqa: E731
    client.on_message("SN12345", cb)
    assert isinstance(client.subscriptions["SN12345"], tuple)
    template, callbacks = client.subscriptions["SN12345"]
...
async def test_dispatch_open_quota_topic_extracts_sn_at_minus_2() -> None:
    """dispatch_message with /open/{user_id}/{sn}/quota routes by SN at position -2."""
    client = MqttTransport(MQTT_CREDS)
    received: list[str] = []
    client.on_message("BK11DEVICE", lambda sn, data: received.append(sn))
    await client.dispatch_message("/open/user456/BK11DEVICE/quota", {"key": "val"})
...
async def test_dispatch_legacy_app_topic_still_works() -> None:
    ...
    await client.dispatch_message("/app/device/property/SN12345", {"soc": 42})
