...
Covers the _WATTS_RAW_FACTOR quirk (3-place rule), from_mqtt_payload with a
...
def test_watts_raw_factor_is_documented() -> None:
    """_WATTS_RAW_FACTOR is 0.1 — raw 'watts' field is 10× actual wattage.
...
def test_smart_plug_from_quota_vector() -> None:
    ...
    assert plug.power_watts == pytest.approx(expected["power_watts"])
    assert plug.voltage == pytest.approx(expected["voltage"])
    assert plug.current == pytest.approx(expected["current"])
    assert plug.temp == pytest.approx(expected["temp"])
...
def test_smart_plug_quota_voltage_no_factor() -> None:
    ...
    """
    payload = {"2_1.volt": 242}
    plug = SmartPlugData.from_quota_payload("SP00002", payload)
    assert plug.voltage == pytest.approx(242.0)
